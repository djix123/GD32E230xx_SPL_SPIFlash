...
uint8_t W25Qx_WriteEnable(void)
{
	...
	while(W25Qx_GetStatus() == W25Qx_BUSY)
	{
		...
		if(tickstart > W25QXXXX_TIMEOUT_VALUE)
		{        
...
void W25Qx_Read_ID(uint16_t *ID)
{
	...
	W25Qx_Enable();
	/* Send the read ID command */
	spi_data_transmit(SPI0, cmd, 4);
	/* Reception of the data */
	spi_data_receive(SPI0, idt, 2);
...
uint8_t W25Qx_Get_Parameter(W25Qx_Parameter *Para)
{
	...
	Para->FLASH_ID = id;
	Para->FLASH_CAPACITY = size;
	Para->SUBSECTOR_COUNT = Para->FLASH_CAPACITY / Para->SUBSECTOR_SIZE;
	Para->SECTOR_COUNT = Para->FLASH_CAPACITY / Para->SECTOR_SIZE;
...
//     return W25Qx_ERROR;
//   }
	spi_data_receive(SPI0, pData, Size);
...
uint8_t W25Qx_Write(uint8_t* pData, uint32_t WriteAddr, uint32_t Size)
{
  ...
  do
  {
		...
		spi_data_transmit(SPI0, pData, current_size);
		W25Qx_Disable();
...
uint8_t W25Qx_Erase_Block(uint32_t Address)
{
	...
	{
		/* Check for the Timeout */
		if(tickstart > W25QXXXX_SECTOR_ERASE_MAX_TIME)
		{        
				return W25Qx_TIMEOUT;
		}
	    delay_1ms(1);
...
uint8_t W25Qx_Erase_Chip(void)
{
	...
	{
		/* Check for the Timeout */
		if(tickstart > W25QXXXX_BULK_ERASE_MAX_TIME)
		{        
			return W25Qx_TIMEOUT;
		}
		delay_1ms(1);
